...
- **Adjoint-square follow-through:** `J_NS=-alpha_t+d varphi` is an orthogonal Hodge split, so `||J_NS||^2=||u_t||^2+||grad varphi||^2`.  Combining with the current-interference law gives `2nu int omega.S.omega=nu^2||curl omega||^2+||u cross omega||^2-||u_t||^2-||grad varphi||^2`, hence `2nu Zdot=||u cross omega||^2-||u_t||^2-||grad varphi||^2-nu^2||curl omega||^2`.  Positive stretching therefore has simultaneous acceleration, pressure-reaction and viscous-roughness costs, though no universal favorable sign.
- **Inverse-mobility stop:** the frozen mobility `M_alpha=(1/nu)P R_alpha^*R_alpha P` has principal symbol `nu|xi|^2 P_xi`, so its inverse action is only order `-2`.  Exact Beltrami heat modes obey `M_alpha alpha=nu N^2 alpha` and have total inverse-mobility action exactly `E(0)` for every `N`, while `K/E=N`.  Finite mobility action therefore cannot be the missing no-escape criterion; the unresolved mechanism must use the lower-order self-curvature coupling rather than the principal elliptic square alone.
