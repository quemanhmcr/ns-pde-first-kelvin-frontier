...
SEAMS = {
    ...
    "cross-event-principal-axis-lineage": ("open-literal", "endpoint spectral projector blocks are canonical but individual principal-axis matching across reset/refinement needs an explicit physical transport map and is gauge-obstructed at degeneracy"),
    "first-bad-physical-residual-refinement-lift": ("open-literal", "generic A tensor A synthesis is exact once A is supplied, but the actual moving first-bad current-to-reconstructed-residual refinement map is not derived line by line"),
    ...
    "first-bad-selected-spectral-lineage": ("open-literal", "badness/resolve predicates, moving cut time faces, actual residual refinement lift, support locality, and cross-clock ancestry remain missing from the literal selected lineage"),
...
for seam in ("cross-event-principal-axis-lineage", "first-bad-physical-residual-refinement-lift", "first-bad-selected-spectral-lineage"):
    if SEAMS[seam][0] != "open-literal":
        raise SystemExit(f"{seam} must remain open-literal until the missing physical map is supplied")
