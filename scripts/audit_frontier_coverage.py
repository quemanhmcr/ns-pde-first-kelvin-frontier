...
SEAMS = {
    ...
    "hysteretic-first-bad-event-structure": ("audited", "for each supplied discrete selector update, literal M_fb is coordinate-piecewise-constant on unresolved intervals and has exact finite entry/resolve pair jump algebra; continuous-time local finiteness is a separate seam"),
    ...
    "adaptive-population-centered-triple-face": ("audited", "general E[C Q C^T] contains the centered triple face E[dC dQ dC^T]; the two-replica four-face law is not a population four-face closure"),
    "own-local-kelvin-affine-target-face": ("audited", "packet-specific local targets force epsilon_P=sum R_i epsilon_i+Delta_omega and r_P=A r_library+d rather than a purely linear residual event"),
    "own-local-kelvin-affine-composition": ("audited", "the target offset d=A Omega_- - Omega_+ is an exact coboundary and telescopes under sequential supplied current events"),
    "own-local-kelvin-affine-moment-selector-faces": ("audited", "own-local second moments and simultaneous selector jumps retain affine offset and linear-target cross faces"),
    "own-local-kelvin-target-gradient-noise-face": ("audited", "own-local Brownian response obeys N_+=A N_-+(A G_- - G_+) with mandatory Gram cross faces"),
    "cubic-heat-shear-own-local-event-no-go": ("audited-calibration", "exact smooth cubic heat-shear NS gives nonzero own-local target mismatch and pure-reanchor qv-source change with A=I"),
    "endogenous-selector-local-finiteness-or-interface-law": ("open-literal", "finite-event hybrid algebra does not prove locally finite endogenous first-bad switching; a summable limit or interface/local-time law is not supplied"),
    "first-bad-own-local-affine-event-instantiation": ("open-literal", "the actual NS-generated packet anchors, target coboundary, event map, and selector timing are not instantiated"),
    ...
    "critical-branch-value-gap-rate": ("audited", "for two differentiable candidate values Delta e=e1-e2 and Delta e dot=e1 dot-e2 dot exactly"),
    "critical-branch-three-face-competition": ("audited-conditional", "for enstrophy critical candidates the ranking-gap rate is the difference of stretching, Kelvin-bulk, and curvature faces"),
    "critical-value-crossing-vs-hessian-degeneracy": ("audited-generic", "a transverse value tie and Hessian degeneracy are independent event conditions; full negative-definite Hessians can persist through a tie"),
    "three-mode-shear-critical-sheet-crossing": ("audited-calibration", "exact periodic three-mode heat shear has persistent y=0,pi critical sheets whose values cross at t=1/nu with nonzero negative transverse curvature"),
    "three-mode-shear-pure-curvature-competition": ("audited-calibration", "at the crossing stretching and Kelvin bulk vanish and the two negative branch rates are carried entirely by different curvature losses"),
    "critical-value-crossing-no-positive-growth": ("audited-calibration", "both exact NS branch values decrease while the larger-value winner switches transversally"),
    ...
    "intrinsic-normalized-vorticity-unit-ball": ("audited", "V=omega/sqrt(2M) is similarity-neutral and maps into the unit ball; at max-envelope differentiability times it obeys the exact normalized vorticity PDE"),
    "intrinsic-unit-ball-contact-split": ("audited", "at active maxima Hc=-V dot Hess(V)=(grad V)^T grad V+Q/2, hence ker Hc=ker Q intersect ker(grad V)"),
    "intrinsic-unit-ball-kelvin-gram-duality": ("audited", "the left Gram of grad V is orientation-complete Kelvin qv divided by 4nuM and the right Gram has the same bulk trace"),
    "intrinsic-elliptic-polarization-contact-transfer": ("audited-calibration", "an exact periodic heat-shear family transfers contact from scalar curvature to orientation twist while Hc_zz=k^2 stays fixed"),
    "intrinsic-scalar-jet-exhaustion-no-go": ("audited-calibration", "the exact helical endpoint has g identically one and every scalar g jet zero while grad V, contact, and Kelvin bulk remain nonzero"),
    "first-bad-intrinsic-contact-kernel-closure": ("open-literal", "no theorem identifies genuine first-bad failure with loss of unit-ball contact coercivity or closes ker Hc by higher vector contact/material symmetry"),
    "first-bad-intrinsic-kernel-closure": ("open-literal", "scalar curvature-kernel closure alone is insufficient; the sharper unresolved seam is unit-ball contact-kernel closure at an actual first-bad state"),
    ...
    "intrinsic-chamber-quadrupole-support-recovery": ("audited-calibration", "orientation-complete face quadrupoles reconstruct the noncollapsing transverse support that the Kelvin flux channel misses"),
    "first-bad-intrinsic-tangential-support-collapse": ("open-literal", "no theorem yet couples an actual first-bad intrinsic compatibility obstruction to tangential support/refinement collapse"),
...
required |= {
    ...
    "intrinsic-normalized-vorticity-unit-ball",
    "intrinsic-unit-ball-contact-split",
    "intrinsic-unit-ball-kelvin-gram-duality",
    "intrinsic-elliptic-polarization-contact-transfer",
    "intrinsic-scalar-jet-exhaustion-no-go",
    "first-bad-intrinsic-contact-kernel-closure",
    ...
    "intrinsic-chamber-quadrupole-support-recovery",
    "first-bad-intrinsic-tangential-support-collapse",
...
for seam in ("adaptive-population-centered-triple-face", "own-local-kelvin-affine-target-face", "own-local-kelvin-affine-composition", "own-local-kelvin-affine-moment-selector-faces", "own-local-kelvin-target-gradient-noise-face"):
    if SEAMS[seam][0] != "audited":
        raise SystemExit(f"{seam} must remain exact audited own-local/population algebra")
if SEAMS["cubic-heat-shear-own-local-event-no-go"][0] != "audited-calibration":
    raise SystemExit("cubic own-local heat-shear no-go must remain an exact audited calibration")
for seam in ("endogenous-selector-local-finiteness-or-interface-law", "first-bad-own-local-affine-event-instantiation"):
    if SEAMS[seam][0] != "open-literal":
...
for seam in ("critical-branch-value-gap-rate", "selected-max-tie-continuity-derivative-switch", "critical-event-taxonomy-ranking-degeneracy-packet"):
    if SEAMS[seam][0] != "audited":
        raise SystemExit(f"{seam} must remain exact audited branch-selection algebra")
if SEAMS["critical-branch-three-face-competition"][0] != "audited-conditional":
    raise SystemExit("critical branch three-face competition must remain conditional on an enstrophy critical ranking observable")
if SEAMS["critical-value-crossing-vs-hessian-degeneracy"][0] != "audited-generic":
    raise SystemExit("value crossing vs Hessian degeneracy must remain a generic geometry separation, not an NS-specific claim")
for seam in ("three-mode-shear-critical-sheet-crossing", "three-mode-shear-pure-curvature-competition", "critical-value-crossing-no-positive-growth"):
    if SEAMS[seam][0] != "audited-calibration":
