...
intrinsic_curvature_support_note = ROOT / "docs" / "intrinsic_curvature_support_grammar_audit.md"
intrinsic_normalized_vorticity_ball_note = ROOT / "docs" / "intrinsic_normalized_vorticity_ball_audit.md"
...
intrinsic_curvature_support_text = intrinsic_curvature_support_note.read_text()
intrinsic_normalized_vorticity_ball_text = intrinsic_normalized_vorticity_ball_note.read_text()
...
random_event_required = [
    ...
    "Fixed event map is a special closure domain",
    "Same payload but random event map leaves pure dispersion",
    "PSD aligned and anti-aligned calibrations",
    "same law applies to continuous q.v. Gram payloads",
    "actual first-bad adaptive event/joint-law instantiation",
    "No restart/continuation/regularity theorem claimed",
]
...
local_growth_gate_required = [
    "Exact local enstrophy balance",
    "orientation-complete Kelvin q.v. bulk",
    "At a spatial critical point, the PDE has three literal faces",
    "positive margin is necessary but not sufficient for growth",
    "every enstrophy critical point has zero stretching scalar",
    "Exact affine vortex: positive gate is a smooth local mechanism",
    "does not refute the periodic/finite-energy target class",
    "necessary instantaneous growth condition",
    "first-bad local-growth-to-continuation bridge remains Open-literal",
    "No restart/continuation/regularity theorem claimed",
]
local_growth_gate_missing = [token for token in local_growth_gate_required if token not in local_growth_gate_text]
if local_growth_gate_missing:
...
critical_hessian_required = [
    "Exact Hessian evolution on a moving critical branch",
    "local connection splits into strain reshaping and rigid rotation",
    "Curvature-volume rate and the incompressible cancellation",
    "Zero determinant-rate is not zero Hessian dynamics",
    "Incompressible critical curvature-volume law",
    "Jacobi determinant law",
    "Conditional nondegeneracy consequence",
    "Exact periodic ABC calibration",
    "Hessian degeneracy is a real theorem boundary",
    "first-bad critical-branch degeneracy identification remains Open-literal",
    "No restart/continuation/regularity theorem claimed",
]
critical_hessian_missing = [token for token in critical_hessian_required if token not in critical_hessian_text]
if critical_hessian_missing:
    print("missing critical-Hessian curvature-volume markers:", critical_hessian_missing)
    sys.exit(1)

branch_competition_required = [
    "Two-branch value gap",
    "branch-rate difference keeps the three physical faces",
    "Value crossing and Hessian degeneracy are different event surfaces",
    "Exact periodic Navier--Stokes crossing calibration",
    "critical sheets",
    "Winner switch is transverse",
    "crossing is pure curvature-rate competition",
    "selected maximum is continuous but its derivative can switch",
    "Hysteresis is a separate rule layer",
    "branch value crossing",
    "first-bad branch-competition/hysteresis identification remains Open-literal",
    "No restart/continuation/regularity theorem claimed",
]
branch_competition_missing = [token for token in branch_competition_required if token not in branch_competition_text]
if branch_competition_missing:
    print("missing enstrophy branch-competition markers:", branch_competition_missing)
    sys.exit(1)

critical_merger_kelvin_required = [
    "Exact smooth Navier--Stokes merger mechanism",
    "A physical orientation-complete Kelvin packet library",
    "Fixed-shape translated packets really do coalesce",
    "Scalar/critical coalescence does not force full packet coalescence",
    "The physical event map for the instantiated branch-resolved library",
    "Same-replica cross blocks at the collision",
    "Endogenous selector/interface behavior in this exact merger",
    "general selector local finiteness remains Open-literal",
    "No restart/continuation/regularity theorem claimed",
]
critical_merger_kelvin_missing = [token for token in critical_merger_kelvin_required if token not in critical_merger_kelvin_text]
if critical_merger_kelvin_missing:
    print("missing critical-sheet merger Kelvin-event markers:", critical_merger_kelvin_missing)
    sys.exit(1)

critical_transport_nanson_required = [
    "The critical sheets are not material sheets",
    "A literal stochastic Kelvin anchor is also not the critical path",
    "The local Nanson connection remembers the branch history",
    "Nanson consistency is exact, but it does not make the packet material",
    "The transported support remembers more than the Kelvin residual fiber",
    "The singular packet derivative is exactly a moving-cut circulation flux",
    "The singular selector speed has finite total variation",
    "What bottleneck this resolves",
    "Open-literal",
    "No continuation/restart/regularity theorem claimed",
]
critical_transport_nanson_missing = [token for token in critical_transport_nanson_required if token not in critical_transport_nanson_text]
if critical_transport_nanson_missing:
    print("missing critical-sheet transport/Nanson markers:", critical_transport_nanson_missing)
    sys.exit(1)

kelvin_ancestry_readout_required = [
    "Keep the two physical layers separate",
    "Exact three-layer covariance law",
    "Moving boundary law: signed revaluation, not a new stochastic producer",
    "Exact Navier--Stokes calibration: a stationary Kelvin ancestry population",
    "The critical chamber is a literal sub-Markov readout",
    "Exact vorticity payoff: criticality regularizes the normalized mean",
    "Selected covariance: Kelvin bulk plus moving-cut revaluation",
    "What bottleneck this resolves",
    "Open-literal",
    "No continuation/restart/regularity theorem claimed",
]
kelvin_ancestry_readout_missing = [token for token in kelvin_ancestry_readout_required if token not in kelvin_ancestry_readout_text]
if kelvin_ancestry_readout_missing:
...
intrinsic_curvature_support_required = [
    "The local normalized scalar law",
    "Why the connection is not a source of localization",
    "Physical support is the dual representation",
    "The global normalization disappears on the flat subspace",
    "The PSD tangent-cone compatibility law",
    "Exact NS referee II: diffusion closes a quartic flat direction",
    "Open-literal:",
    "No restart/continuation/regularity theorem claimed",
]
intrinsic_curvature_support_missing = [token for token in intrinsic_curvature_support_required if token not in intrinsic_curvature_support_text]
if intrinsic_curvature_support_missing:
    print("missing intrinsic curvature-support grammar markers:", intrinsic_curvature_support_missing)
    sys.exit(1)

intrinsic_normalized_vorticity_ball_required = [
    "Normalize the vector, not only its norm",
    "Exact normalized-vorticity PDE",
    "Contact-kernel completeness",
    "Kelvin q.v. is already the same gradient sector",
    "Exact NS polarization family",
    "Two endpoints expose the scalar-jet no-go",
    "Open-literal:",
    "No restart/continuation/regularity theorem claimed",
]
intrinsic_normalized_vorticity_ball_missing = [token for token in intrinsic_normalized_vorticity_ball_required if token not in intrinsic_normalized_vorticity_ball_text]
if intrinsic_normalized_vorticity_ball_missing:
    print("missing intrinsic normalized-vorticity ball markers:", intrinsic_normalized_vorticity_ball_missing)
    sys.exit(1)
...
for forbidden in [
    ...
    "unit-ball contact proves continuation",
    "contact coercivity is established at first bad",
    "ker contact is empty at first bad",
    "PSD kernel viability proves continuation",
    "intrinsic curvature-support grammar proves regularity",
    "diffusive kernel birth proves support collapse",
    "kernel compression defines actual first-bad event",
    ...
    "mean event map times mean payload closes adaptive first-bad",
    ...
    "positive local growth gate certifies singularity",
    "positive local growth gate certifies continuation failure",
    "ABC disproves the local-max growth gate",
    ...
    "incompressibility freezes Hessian eigenvalues",
    "incompressibility freezes Hessian shape",
    "local strain cannot change Hessian shape",
    "finite Hessian logdet integral proves Navier-Stokes regularity",
    "critical branch continuation is Navier-Stokes continuation",
    "critical Hessian nondegeneracy proves restart",
    "Hessian degeneracy is the singularity event",
    "winner switch implies Hessian degeneracy",
    "winner switch requires positive local growth",
    "critical value tie certifies singularity",
    "three-mode shear has a full nondegenerate 3D Hessian",
    "branch value crossing is a physical packet refinement event",
    "selected maximum is differentiable through every transverse crossing",
    "critical-sheet merger forces full Kelvin packet-state coalescence",
    "critical merger canonically fixes packet shape",
    "critical merger identifies branch ancestry",
    "critical merger proves continuation",
