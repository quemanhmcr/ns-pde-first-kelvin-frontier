...
On a frozen physical library and on **one compatible Markov clock** `sigma`, the
fixed-current conditional covariance law is

\[
\partial_\sigma C=-\Gamma_K.
\]

For a selector coefficient path parameterized by that same clock,

\[
\boxed{
\frac d{d\sigma}V_\sigma(a_\sigma)
=-\Gamma_{K,\sigma}(a_\sigma,a_\sigma)
+2C_\sigma(a_\sigma,\partial_\sigma a_\sigma).
}
\]

The second term is the exact signed covariance work of selector motion and is not
observer-generated stochastic q.v.

However the programme's first-bad selector is parameterized by **physical time**
`t`, while the causal Kelvin bank naturally also carries a backward/remaining
horizon `tau`.  For a two-clock covariance matrix `C(t,tau)` the literal chain rule
is

\[
\boxed{
\frac d{dt}\big[a_t^TC(t,\tau(t))a_t\big]
=a_t^T\big(\partial_t C+\dot\tau\,\partial_\tau C\big)a_t
+2a_t^TC\dot a_t.
}
\]

Thus the older one-clock physical-time telescope is not certified merely from
`partial_sigma C=-Gamma_K`.  One must first construct the state/time map relating
the physical first-bad path to the ancestry/backward-Kelvin bank.  The same-clock
identity above remains exact; its physical two-clock lift remains open-literal.
