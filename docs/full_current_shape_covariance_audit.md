...
## 13. Follow-through: the finite-shape descent error has now been derived literally

The next-target shorthand `K_{Z(R)}-K_{Z_local(D,H)}` was still overtyped because
actual reverse-current area geometry and the Cauchy metric-dual packet frame have
opposite connection signs.  The corrected local Stokes comparison uses the actual
finite-surface area vector `h_R`:

\[
\varepsilon_K
=K_{Z(R)}-\omega(X)\cdot h_R.
\]

On the full reverse-age state it obeys exactly

\[
d\varepsilon_K
=-\omega\cdot\mathcal R_A\,d\sigma
+\sqrt{2\nu}\sum_\mu q_\mu^{\rm err}dW^\mu,
\]

where `mathcal R_A` is the finite strain-gradient shape current and
`q_mu^err` is the finite-support vorticity-gradient residual.  Pathwise
`[vec D,epsilon_K]=0`, while finite-horizon `Cov(vec D,epsilon_K)` is the ordinary
off-diagonal connected-covariance block generated by common anchor sampling.

Exact cubic heat shear gives `epsilon_K != 0` with zero drift, zero q.v., and zero
`D` covariance, proving that covariance alone cannot force finite-shape descent.
Exact ABC activates the finite-variation shape drift, and exact one-mode shear
activates the stochastic error q.v. and `D` cross covariance.  The Legendre heat
shear hierarchy still defeats every finite moment truncation.

**Status: Exact identity / audited NS calibrations.  Uniform first-bad
shrinking-support descent remains Open-literal.**
